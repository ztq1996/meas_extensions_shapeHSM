 ...
 */
void nrerrorOld(const char error_text[])
...
/* nrerror
 * *** replacement for NUMERICAL RECIPES ERROR HANDLER ***
 * This code is in the public domain.
 */
...
void find_mom_1(RECT_IMAGE *data, FTYPE **moments, int max_order,
...
void find_mom_2(RECT_IMAGE *data, FTYPE **moments, int max_order,
...
void convolution_matrix_1(FTYPE **psf_moments, int max_order_psf, int max_order_gal,
   double sigma_f, double sigma_g, double sigma_h, FTYPE **psfmatrix, int **sindex) {
   ...
   Cnml = (FTYPE ***) malloc ((size_t)( (max_order_tot+1) * sizeof(FTYPE**) ));
...
unsigned int general_shear_estimator(RECT_IMAGE *gal_image, RECT_IMAGE *PSF,
   ...
   if (num_iter == NUM_ITER_DEFAULT) {
       return 1;
         ...
         gal_data->mxx = Mxx_gal;
         gal_data->myy = Myy_gal;
         gal_data->mxy = Mxy_gal;
   }
   ...
   if (num_iter == NUM_ITER_DEFAULT) {
       ...
       PSF_data->mxx = Mxx_psf;
       PSF_data->myy = Myy_psf;
       PSF_data->mxy = Mxy_psf;
   }
