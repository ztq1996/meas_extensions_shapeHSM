...
Calls the HSM shape code through the c++ adapter for the LSST meas_algorithms code.
...
def main(x, y, imgFile, psfFile, algorithm, bkgd):
    ...
    hsmShape = hsmLib.HsmShapeAdapterF(exposure, peak)
    status = hsmShape.measure(algorithm)

    ##########################################
    # see how we did
    print "e1,e2,shearSig = ", hsmShape.getE1(), hsmShape.getE2(), hsmShape.getShearSig()
