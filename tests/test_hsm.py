...
round_moments_expected = np.array(
    [  # sigma, e1, e2, flux, x, y
...
class ShapeTestCase(unittest.TestCase):
    ...
    def computeDirectShapeFromGalSim(record, exposure, config):
        ...
        psfBounds = galsim.BoundsI(psfBBox.getMinX(), psfBBox.getMaxX(), psfBBox.getMinY(), psfBBox.getMaxY())
    ...
    def testHsmShape(self):
        ...
        for (algNum, algName), (i, imageid) in itertools.product(
            enumerate(correction_methods), enumerate(file_indices)
        ):
...
class PsfMomentsTestCase(unittest.TestCase):
    ...
    @staticmethod
    def computeDirectPsfMomentsFromGalSim(psf, center, useSourceCentroidOffset=False):
    ...
    def testHsmPsfMoments(self, width, useSourceCentroidOffset, varyBBox, wrongBBox, center):
        psf = PyGaussianPsf(35, 35, width, varyBBox=varyBBox, wrongBBox=wrongBBox)
        ...
        for fieldName in cat.schema.extract("*HsmPsfMoments_[xy]"):
            self.assertEqual(
                cat.schema[fieldName].asField().getDoc(), "Centroid of the PSF via the HSM shape algorithm"
            )
        for fieldName in cat.schema.extract("*HsmPsfMoments_[xy][xy]*"):
            self.assertEqual(
                cat.schema[fieldName].asField().getDoc(),
    ...
    def testHsmPsfMomentsDebiased(self, width, useSourceCentroidOffset, varyBBox, wrongBBox, center):
    ...
    testHsmPsfMomentsDebiasedEdgeArgs = dict(
        width=(2.0, 3.0, 4.0), useSourceCentroidOffset=(True, False), center=((1.2, 1.3), (33.2, 50.1))
    )
